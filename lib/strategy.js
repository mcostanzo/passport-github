...
function Strategy(options, verify) {
  ...
  this._userOrganizationURL = options.userEmailURL || 'https://api.github.com/user/orgs';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
    ...
    self._oauth2.get(self._userEmailURL, accessToken, function (err, body, res) {
      ...
      self._oauth2.get(self._userOrganizationURL, accessToken, function (err, body, res) {

        if (err)
          return done(new InternalOAuthError('Failed to fetch user organizations', err));

        profile.organizations = JSON.parse(body);
